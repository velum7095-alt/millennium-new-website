  ...
  return (
    <div>
      ...
      <div className="glass p-6 mb-8">
        <h3 className="text-slate-900 font-medium mb-4">{editingId ? 'Edit Product' : 'Add New Product'}</h3>
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-red-500/50" placeholder="Product Name" />
          <input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-red-500/50" placeholder="Brand" />
          ...
          <input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-red-500/50" placeholder="Price" />
        ...
        <div className="mb-4">
          ...
          {form.features.map((f, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <input value={f} onChange={(e) => updateFeature(i, e.target.value)} className="flex-1 px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-red-500/50" placeholder="Feature" />
        ...
        {editingId ? (
          <button onClick={handleUpdate} className="gradient-bg px-6 py-2 rounded-lg text-slate-900 text-sm font-medium flex items-center gap-2"><Save className="w-4 h-4" /> Update Product</button>
        ) : (
          <button onClick={handleAdd} className="gradient-bg px-6 py-2 rounded-lg text-slate-900 text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" /> Add Product</button>
        )}
      </div>
        ...
        {products.map((product) => (
          <div key={product.id} className="glass p-4 flex items-center justify-between card-hover">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-50 rounded-lg flex items-center justify-center text-white/20 font-bold">{product.brand.charAt(0)}</div>
              <div>
                <p className="text-slate-900 font-medium text-sm">{product.name}</p>
                <p className="text-text-secondary text-xs">{product.brand} | {product.category} {product.isFeatured && '| Featured'}</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => handleEdit(product)} className="w-8 h-8 glass rounded-lg flex items-center justify-center text-text-secondary hover:text-slate-900"><Edit3 className="w-4 h-4" /></button>
