  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Channel Partners</h1>
      ...
      <div className="glass p-6 mb-8">
        <h3 className="text-slate-900 font-medium mb-4">{editingId ? 'Edit Partner' : 'Add New Partner'}</h3>
        <div className="grid md:grid-cols-3 gap-4">
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-red-500/50" placeholder="Partner Name" />
          ...
          <div key={partner.id} className="glass p-4 flex items-center justify-between card-hover">
            <div className="flex items-center gap-3">
              {partner.logo ? (
                ...
                <div className="w-12 h-12 gradient-bg rounded-lg flex items-center justify-center text-slate-900 font-bold">{partner.name.charAt(0)}</div>
              )}
              <div>
                <p className="text-slate-900 font-medium text-sm">{partner.name}</p>
                {partner.isAuthorized && <span className="text-accent-gold text-xs">Authorized</span>}
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => handleEdit(partner)} className="w-8 h-8 glass rounded-lg flex items-center justify-center text-text-secondary hover:text-slate-900"><Edit3 className="w-4 h-4" /></button>
